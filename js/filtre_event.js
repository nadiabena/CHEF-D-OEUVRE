      ...
      xhr.onreadystatechange = function(){
        if(this.readyState == 4 && this.status == 200){
          if(self[self.selectedIndex].id >= 1){
            ...
            document.getElementById("id_event_by_select").innerHTML = "<br/>" + "<u>" + this.responseText + "</u>";

          } else {
            document.getElementById("id_event_by_select").innerHTML = "";  // Si je selectionne autre choix Tout ou 
          }
        }
      };

      xhr.open("GET", "event_filtre.php?event="+target, true);  //Méthode , UrL, base asynchro
      xhr.send();
    }
